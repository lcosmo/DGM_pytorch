...
//         for(int pi=0; pi< (CHK::DIMX < 1 ? 1 : CHK::DIMX); pi++)
//         {              
//             xi[pi] += 23;
//         }
    }
...
template<typename TYPE, class FUN, class DUMMY>
__global__ void GpuConv1DOnDevice_NoChunks(FUN fun, int nx, int ny, TYPE *out, TYPE **args, DUMMY *d_state) {    
...
//             float r = -log(-log(curand_uniform(d_state + idx)));
//             xi[pi] += r;
//         }
  }
...
__global__ void randuniform(DUMMY *state){
    int idx = threadIdx.x + blockDim.x*blockIdx.x;
...
template < class DUMMY > 
__global__ void setup_kernel(DUMMY *state){
  int idx = threadIdx.x+blockDim.x*blockIdx.x;
...
struct GpuConv1DOnDevice<0,DUMMY> {
    ...
    static void Eval(dim3 gridSize, dim3 blockSize, size_t SharedMem, FUN fun, int nx, int ny, TYPE *out, TYPE **args) {
        ...
        setup_kernel<<<gridSize, blockSize, SharedMem>>>(d_state);
        
//         randuniform<<<gridSize, blockSize>>>(d_state);
            
        printf("\n!!!!! GpuConv1DOnDevice_NoChunks\n");
        GpuConv1DOnDevice_NoChunks <<< gridSize, blockSize, SharedMem >>> (fun, nx, ny, out, args, d_state);
    }
};
