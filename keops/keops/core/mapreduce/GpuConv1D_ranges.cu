...
template < typename TYPE, class FUN, class RANDSTATE >
__global__ void GpuConv1DOnDevice_ranges_NoChunks(FUN fun, int nx, int ny,
    int nbatchdims, int *shapes, int *offsets_d,
    __INDEX__* lookup_d, __INDEX__* slices_x, __INDEX__* ranges_y,
    TYPE *out, TYPE **args, RANDSTATE d_state) {
...
//                             }
                            for(int pi=0; pi< (DIMX < 1 ? 1 : DIMX); pi++)
                            {
                                float r = -log(-log(curand_uniform(d_state + tidx)));
//                                 float r = curand_uniform(d_state + tidx);
                                fout[pi] -= r;
                            }
...
//         cudaMalloc(&d_state, sizeof(curandState));
        curandState *d_state;  cudaMalloc((void**)&d_state, DSIZE*sizeof(curandState));
        ...
        GpuConv1DOnDevice_ranges_NoChunks <<< gridSize, blockSize, SharedMem >>> (fun, nx, ny, nbatchdims, shapes, offsets_d,
    		lookup_d, slices_x, ranges_y, out, args, d_state);
        cudaFree(d_state);
    }
};
